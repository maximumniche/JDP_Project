...
void setup() {
    ...
    //  Audio timer setup
    audioTimer = new HardwareTimer(TIM2); // Create a hardware timer with TIM2
    audioTimer->setOverflow(SAMPLE_RATE, HERTZ_FORMAT); // Set it to get an update event every 1/44,100 seconds
    audioTimer->attachInterrupt(audioISR); // Attach this interrupt to call at each event
    audioTimer->resume(); // Resume audiotimer
...
void loop() {
    ...
    // Write data serially to monitor
